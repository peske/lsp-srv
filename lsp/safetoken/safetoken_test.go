...
func TestTokenOffset(t *testing.T) {
	...
	}, "go/token", "github.com/peske/lsp-srv/lsp/...", "golang.org/x/tools/gopls/...")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, pkg := range pkgs {
		...
		case "go/token":
			tokenPkg = pkg
		case "github.com/peske/lsp-srv/lsp/safetoken":
			safePkg = pkg
		}
	}
	...
	if safePkg == nil {
		t.Fatal("missing package github.com/peske/lsp-srv/lsp/safetoken")
	}
	...
	for _, pkg := range pkgs {
		...
		for ident, obj := range pkg.TypesInfo.Uses {
			...
			t.Errorf(`%s: Unexpected use of (*go/token.File).Offset. Please use github.com/peske/lsp-srv/lsp/safetoken.Offset instead.`, fset.PositionFor(ident.Pos(), false))
		}
	}
}
